...
//debtors.Where(d => "AEIOUaeiou".Contains(d.FullName.FirstOrDefault())).ToList().ForEach(d => Console.WriteLine(d));

//Task2 rhyta.com və ya dayrep.com domenində poçtu olan borcluları göstərin

//debtors.Where(d => d.Email.Contains("rhyta.com") || d.Email.Contains("dayrep.com")).ToList().ForEach(d => Console.WriteLine(d));

//Task3 Yaşı 26-dan 36-ya qədər olan borcluları göstərin.

//debtors.Where(d => d.BirthDay.Year <= 1997 && d.BirthDay.Year >= 1987).ToList().ForEach(d => Console.WriteLine(d));

//Tas4 Borcu 5000-dən çox olmayan borcluları göstərin

//debtors.Where(d => d.Debt < 5000).ToList().ForEach(d => Console.WriteLine(d));

//Task5 Tam adı 18 simvoldan çox olan və telefon nömrəsi 2 və ya daha çox yeddidən ibarət olan borcluları göstərin.

//debtors.Where(d => d.FullName.Length >= 18 && d.Phone.Count(c => c == '7') >= 2).ToList().ForEach(d => Console.WriteLine(d));
